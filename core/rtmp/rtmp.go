...
func HandleConn(c *rtmp.Conn, nc net.Conn) {
  ...
  vodPath := config.Config.VoDLocation + "/vod_" + getCurrentTimeAsString() + ".flv"
  ...
  log.Tracef("Using temporary pipe file at %s", pipePath)

  w := flv.NewMuxer(f)

  var callbacks []func(pkt av.Packet)

  callbacks = append(callbacks, func(pkt av.Packet) {
    if err := w.WritePacket(pkt); err != nil {
      panic(err)
    }
  })

  // VoD is enabled, we write the stream to a file, so it can probably be seen again :)
  // basic recording
  if config.Config.VoDEnabled {
    vf, vfErr := os.OpenFile(vodPath, os.O_CREATE|os.O_RDWR, os.ModeAppend)
    if vfErr != nil {
      panic(vfErr)
    }
    _VoDFile = vf

    log.Infof("Saving the Stream to the following file: %s", vf.Name())
